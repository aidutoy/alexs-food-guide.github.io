...
const RatingDisplay: React.FC<{ label: string; score: number; max: number }> = ({ label, score, max }) => (
    <div className="flex items-center justify-between text-sm">
        ...
        <div className="flex">
            {Array.from({ length: max }, (_, i) => <StarIcon key={i} filled={i < score} />)}
        </div>
    </div>
);
    ...
    <div className="flex items-center justify-between text-sm">
        ...
        <div className="flex text-lg font-semibold">
            {Array.from({ length: max }, (_, i) => (
                <span key={i} className={i < score ? 'text-gray-700' : 'text-gray-300'}>€</span>
            ))}
        </div>
    </div>
);
      ...
      >
        ...
        <div className="w-full md:w-3/5 relative group bg-black">
            ...
            <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/80 to-transparent p-4">
                <p className="text-white text-sm drop-shadow-md text-center mb-4">{allImages[currentImageIndex].caption}</p>
            ...
            {allImages.length > 1 && (
                <>
                    ...
                    <button 
                        onClick={prevImage} 
                        className="absolute top-1/2 left-4 -translate-y-1/2 bg-black/30 hover:bg-black/50 text-white p-2 rounded-full opacity-0 group-hover:opacity-100 transition-opacity z-10"
                    ...
                    <button 
                        onClick={nextImage} 
                        className="absolute top-1/2 right-4 -translate-y-1/2 bg-black/30 hover:bg-black/50 text-white p-2 rounded-full opacity-0 group-hover:opacity-100 transition-opacity z-10"
                     ...
                     <div className="space-y-3 pt-4 border-t">
                        ...
                        <div className="flex items-center space-x-4">
                            {restaurant.websiteUrl && (
                                <a href={restaurant.websiteUrl} target="_blank" rel="noopener noreferrer" aria-label="Website" className="text-gray-500 hover:text-brand-primary transition-colors">
                                    <GlobeIcon className="w-6 h-6" />
                                </a>
                            )}
                            {restaurant.instagramUrl && (
                                <a href={restaurant.instagramUrl} target="_blank" rel="noopener noreferrer" aria-label="Instagram" className="text-gray-500 hover:text-brand-primary transition-colors">
